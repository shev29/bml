 ...
 <!-- Aprroval Cuti -->
           <div class="row">
            ...
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <p class="card-title mb-0">Download Dokumen</p>
...
<?php
error_reporting(0);
 $sql = "SELECT `id_trcuti`, tr_cuti.nik,nama_cuti, nama_lengkap, tr_cuti.kode_jcuti,
 `tgl_pengajuan`, `tgl_awalc`, `tgl_akhir`, `detail_tanggal`,
 `total_hari`, `cuti_tahun`, `alasan`,	doc_pendukung,
 `nik_sleader`, `status_sleader`, `tgl_appsleader`,
 `nik_check1`, `status_check1`, `tgl_check1`, `nik_check2`,
 `status_check2`, `tgl_check2`, `nik_approve1`, `status_approve1`,
 `tgl_approve1`, `nik_approve2`, `status_approve2`, `tgl_approve2`,  
 nik_hrgas, `app_hrgas`, `tgl_apphrgas`, `nik_hrgaspv`, `app_hrgaspv`,
 `tgl_apphrgaspv`, `nik_hrgamng`, `app_hrgamng`, `tgl_apphrgamng`,
 tr_cuti.ket, tr_cuti.ket2 FROM `tr_cuti` 
 INNER JOIN user ON tr_cuti.nik = user.nik
 INNER JOIN jenis_cuti ON tr_cuti.kode_jcuti = jenis_cuti.kode_jcuti
 WHERE tr_cuti.kode_jcuti='CTN1' or tr_cuti.kode_jcuti='CM90' or tr_cuti.kode_jcuti='CK45' or tr_cuti.kode_jcuti='CTSKT'  

 GROUP BY id_trcuti order by id_trcuti DESC";
$result = $conn->query($sql);
 //WHERE kode_jcuti !='CTSKT'
...
$warnaHeading = "#FF0000";
  ...
  while($row = $result->fetch_assoc()) {
	  ...
	  $data = explode("." , $doc_pendukung);
	  $docs = $data[1];
	  if (!empty($doc_pendukung)){
		    $links="<center><a href='doc_pendukung/$doc_pendukung'><button type='button' class='btn btn-inverse-warning btn-fw'>View</button></a>
			<br><a href='download.php?filename=$doc_pendukung'><label style='cursor: pointer;' class='badge badge-success'>
			Download</label></a>";
		  if (empty($docs)){
			 //<img src='assets/images/nodoc.png' width='40' height='40'>
			  $docs ="
			  <label class='badge badge-danger'> Belum Upload Doc</label>";
			
		  }
		  elseif($docs) {
 
				$docs ="<font style='font-size: 10px;'>$doc_pendukung</font>";
		  }
	  }
	  elseif(empty($doc_pendukung)){
		  $docs="<center><button type='button' class='btn btn-inverse-info btn-fw'>Tidak Wajib Doc</button></center>";
		  $links="-";
	  }
	...
	echo "<tr bgcolor='$warna'>
			<td><font style='font-size: 10px;'>$id_trcuti</font></td>
			<td><font style='font-size: 10px;'>$nmt_new</font></td>
			<td><font style='font-size: 10px;'>$nama_cuti</font></td>
			<td><font style='font-size: 10px;'>$alasan</font></td> 
			<td><font style='font-size: 10px;'>$tgl_ajuan</font></td>  
			<td><font style='font-size: 10px;'>$docs</font></td>
			<td><font style='font-size: 10px;'><center>$links</center></font></td>
