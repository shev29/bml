...
// Email Notification
	...
	(select nama_lengkap from user where nik=check_2)as nm_check1, (select email from user where nik=check_2)as email_c1, 
	(select email2 from user where nik=check_2)as email_c12, (select username from user where nik=check_2)as username_c1,
	(select token from user where nik=check_2)as token_c1 FROM `tr_cutimail` INNER JOIN user ON tr_cutimail.nik = user.nik 
...
if ($resultemailz->num_rows > 0) {
  ...
  while($rowemailz = $resultemailz->fetch_assoc()) {
	  ...
	  $nm_kecil = strtolower($nama_pemohon);
	   $mail->addAddress("krisman.silalahi.pnb@logisteed.com","");
	   $mail->addAddress("itstaff.acb@logisteed.com","");
	//     $mail->addAddress("nugroho@logisteed.com","");
...
/*	$mail->addAddress("dudi.ramdani@hts-bml.co.id",""); //email SPV 
	...
	$bodys.="<tr><td colspan='3'>Dear $nm_check1 as Approver 1b</td></tr> 
	...
	$bodys.="<tr><td colspan='3'>http://bml-log.com/verify.php?id_trcuti=$id_trcuti&&user=$username_c1&nikp=$nik_atasan2&&token=$token_c1</td></tr>";
	
	$bodys.="<tr><td colspan='3'><br>For more detail</td></tr>";
    $bodys.="<tr><td colspan='3'>Silahkan Login untuk akses penuh / Please Login for full access https://bml-log.com/</td></tr>";
 ...
 $sql = "UPDATE `user` SET `exp_token` = DATE_ADD(NOW(), INTERVAL 7 DAY) WHERE nik='$nik_atasan2'";
 //var_dump($sql);
