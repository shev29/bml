 ...
 <!-- Aprroval Cuti -->
           ...
           <div class="row">
            ...
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <p class="card-title mb-0">History Cuti <?php //echo $niks; ?> </p>
...
<?php
 ...
 `tgl_appsleader`, `nik_check1`,
 (select nama_lengkap from user where nik=nik_check1)as nm_nikcheck1,
 `status_check1`, `tgl_check1`, `nik_check2`, `status_check2`, `tgl_check2`, `nik_approve1`, 
 
 (select nama_lengkap from user where nik=nik_approve1)as nm_approve1,
 `status_approve1`, `tgl_approve1`, `nik_approve2`,
 `status_approve2`, `tgl_approve2`,
  (select nama_lengkap from user where nik=nik_approve1)as nm_approve1,
 `nik_hrgas`, `app_hrgas`, `tgl_apphrgas`,
   (select nama_lengkap from user where nik=nik_hrgas)as nm_hrgastaff,
 ...
 INNER JOIN jenis_cuti ON 
 tr_cuti.kode_jcuti = jenis_cuti.kode_jcuti 
 WHERE 
 tr_cuti.nik='$nikso'
...
$warnaHeading = "#FF0000";
  ...
  while($row = $result->fetch_assoc()) {
	  ...
	  else $app_sleader="<div class='badge badge-warning'>Waiting</div>"; 
	  
	  $tgl_appsleader = $row['tgl_appsleader'];
	  $approve_spv = $row['status_check1'];
	  	// Approve  SPV
	  if ($approve_spv=='1')
		  $app_spv="<div class='badge badge-success'>$nm_nikcheck1 <br></div><p style='font-size:10px'><i>$tgl_check1</i></p>";
	  else $app_spv="<div class='badge badge-warning'>Waiting</div>";
	  
	  $tgl_appmanager = $row['tgl_appmanager'];
	  $app_manager = $row['status_approve1'];
	  	// Approve  SPV
	  if ($app_manager=='1')
		  $approve_manager="<div class='badge badge-success'>$nm_approve1 <br></div><p style='font-size:10px'><i>$tgl_approve1</i></p>";
	  else $approve_manager="<div class='badge badge-warning'>Waiting</div>";
	  
	  $tgl_apphrgas = $row['tgl_apphrgas'];
	  $app_hrgas = $row['app_hrgas'];
	  $nm_hrgastaff = $row['nm_hrgastaff'];	
	  // Approve HRGA Spv
	  	  if ($app_hrgas=='1')
		  $approve_hrgas="<div class='badge badge-success'>$nm_hrgastaff</div><p style='font-size:10px'><i>$tgl_apphrgas</i></p>";
	  else $approve_hrgas="<div class='badge badge-warning'>Waiting</div>";
	  ...
	  // Approve HRGA Spv
	  	  if ($app_hrgaspv=='1')
		  $approve_hrgaspv="<div class='badge badge-success'> $nm_hrgaspv</div><p style='font-size:10px'><i>$tgl_apphrgaspv</i></p>";
	  else $approve_hrgaspv="<div class='badge badge-warning'>Waiting</div>";
	  ...
	  if ($app_hrgamng=='1')
		  $approve_hrgamng="<div class='badge badge-success'>$nm_hrgamng</div><p style='font-size:10px'><i>$tgl_apphrgaspv</i></p>";
	  else $approve_hrgamng="<div class='badge badge-warning'>Waiting</div>";
	...
	echo "<tr bgcolor='$warna'> 
			...
			<td>
				<div class='dropdown'>
                      ...
                      <div class='dropdown-menu' aria-labelledby='dropdownMenuSizeButton2'> 
                        ...
                        <div class='dropdown-divider'>	</div>                        
                        <a class='dropdown-item' href='#'>Lama : $newtgl_awalc - $newtgl_akhir ($total_hari) Hari</a> 
						<div class='dropdown-divider'>	</div>                                            
                        <a class='dropdown-item' href='#'>Alasan/Keperluan : $alasan</a> <br><br>   
                      </div>
                    </div>
			...
			?>
				 <td> <div class='badge badge-warning'>Waiting</div> 
				</td> 
			<?php }
		}
				else{
