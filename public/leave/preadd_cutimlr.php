                ...
                <div class="card-body"> 
				   <div class="form-group row">
					...
					<?php  include "assets/configure/koneksi.php"; 
							$resultkrw = mysqli_query($conn, "SELECT nama_lengkap, nik FROM `user` 
															  WHERE kode_loc like '%KARAWANG%' AND nik='$nikso' ");
					 
						// tampilkan query
							$rowkrw=mysqli_fetch_row($resultkrw); 
							$nama_lengkap=$rowkrw[0];
							$nik=$rowkrw[1];
							if(!empty($rowkrw)){
                    ...
                    <div class="form-group row">
                     <label for="exampleInputMobile" class="col-sm-3 col-form-label">Shift Leader</label>
                      <div class="col-sm-9"> <select name='nik_sleader' class="js-example-basic-multiple w-100"> 
				<?php
					date_default_timezone_set("Asia/Bangkok");
					$nowdate_forexpire = date('Y-m-d');
					
					include "assets/configure/koneksi.php";  
					 $sql = "SELECT nama_lengkap, nik FROM `user` WHERE ket2='Shift Leader'";
 					$result = $conn->query($sql);
					...
					if ($result->num_rows > 0) {
					  ...
					  echo "
					  <option value='$nik'>$nama_lengkap</option>";
					  }
					} else {
					  echo "0 results";
					}
