...
// Email Notification
	...
	(select nama_lengkap from user where nik=check_1)as nm_check1, (select email from user where nik=check_1)as email_c1, 
	(select email2 from user where nik=check_1)as email_c12, (select username from user where nik=check_1)as username_c1,
	(select token from user where nik=check_1)as token_c1 FROM `tr_cutimail` INNER JOIN user ON tr_cutimail.nik = user.nik 
...
if ($resultemailz->num_rows > 0) {
  ...
  while($rowemailz = $resultemailz->fetch_assoc()) {
	...
	$bodys.="<tr><td colspan='3'>Dear $nm_check1 as Approver 1</td></tr> 
	...
	$bodys.="<tr><td colspan='3'>http://bml-log.com/verify.php?id_trcuti=$id_trcuti&&user=$username_c1&nikp=$nik_atasan&&token=$token_c1</td></tr>";
...
//} 
// notif email
//  header('Location:getmail_lev1b.php?id_b='.$getid);
