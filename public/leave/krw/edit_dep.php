 ...
 <!-- form -->
           <div class="row"> 
            <div class="col-md-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Edit Data Department</h4>
					<!-- `id_dep`, `kode_dep`, `nama_dep`, `ket` -->
					
					<?php 
						include "assets/configure/koneksi.php"; 
						error_reporting(0);
						$get_id_dep = $_GET['id_dep'];
						$result6 = mysqli_query($conn, "SELECT  `id_dep`, `kode_dep`, `nama_dep`, `ket` 
														FROM `department`  where id_dep='$get_id_dep'
														");
					// var_dump($get_idloc);
						// tampilkan query
						 $row6=mysqli_fetch_row($result6);
							...
							?>
                   <form class="forms-sample" method='post' action='update_dep.php'>
                    <div class="form-group row">
                      <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Kode Dept</label>
                      <div class="col-sm-9">
					   <input type="hidden" name='id_dep' value='<?php echo $get_id_dep; ?>' class="form-control" id="exampleInputUsername2"  >
                      
                        <input type="text" name='kode_dep' value='<?php echo $kode_dep;?>' class="form-control" id="exampleInputUsername2" placeholder='(TB8...)' >
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Nama Dept</label>
                      <div class="col-sm-9">
                        <input type="text" class="form-control" name='nama_dep' value='<?php echo $nama_dep;?>' id="exampleInputUsername2"  >
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Ket</label>
                      <div class="col-sm-9">
                        <input type="text" name='ket' value='<?php echo $ket;?>' class="form-control" id="exampleInputUsername2" >
                      </div>
                    </div>
