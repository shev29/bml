...
$sql = "SELECT `no_order`, `type_order`, `nama_barang`, `job_refnumber`, (courierm_alamat.dtl_alamat) as alamat_asal,  
		...
		WHERE   type_order='Pickup' 
		AND  nik_kurir='$nikkur'
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
			...
			if(!empty($status==="Waiting Courier")){
				...
				$tag_move = "<a href='ambil.php?resi=$no_order'> <button type='submit' class='btn btn-block btn-success btn-sm'>
                  Pickup
                </button></a>";
...
?>                  
		...
		if ($result->num_rows > 0) {
		  ...
		  while($row = $result->fetch_assoc()) {
			...
			if(!empty($status==="Waiting Courier")){
				$tag="<span class='badge badge-secondary'>Mng Kurir</span>";	
				$tag_move = "<a href='ambil_send.php?resi=$no_order'><button type='submit' class='btn btn-block btn-success btn-sm'> 
				Send
				</button></a>";
							 				
			}
					if(!empty($status==="Requested")){
				$tag="<span class='badge badge-secondary'>Requested</span>";	
				$tag_move = "<a href='ambil_send.php?resi=$no_order'><button type='submit' class='btn btn-block btn-success btn-sm'> 
				Send
				</button></a>";
				...
				$tag_move="$status";
					...
					elseif(!empty($status=="Transfer")){
						...
						echo "<tr>
							...
							<td> 
							 <div class='btn-group'><p style='font-size:70%;'> $pt_tujuan ($dept_penerima )  </p>  
								  <button type='button'  data-toggle='dropdown'> +
								  <div class='dropdown-menu'><p style='font-size:70%;'>&nbsp;&nbsp;&nbsp;$alamat_tujuan</p>  
								  </div>
								</div>
